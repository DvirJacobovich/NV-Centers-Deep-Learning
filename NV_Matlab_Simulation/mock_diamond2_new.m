classdef mock_diamond2_new < handle
    ...
    properties
%         num_pts = 180; % number of frequency points in simulation
        num_pts;
        real_num_poits;
        curr_cs_per;
        df;
    ...
    methods
        function obj = mock_diamond2_new(df)
        obj.B_theta = 30;
        obj.B_phi = 60;
...
%                 df = 2; % gap between every two poits in a MHz.
%         df = 2; % gap between every two poits in a MHz.
...
%             failsafe if small magnetic field projections
      ...
      obj.num_pts = ceil((full_peak_window+2*pad_width)/obj.df);
      obj.curr_cs_per = (50 / obj.num_pts) * 100;
        % The center is at 2785 for all magnetic fields. Lets define a
        % fixed the FULL window size for all to be 800, so the pad_width is:
