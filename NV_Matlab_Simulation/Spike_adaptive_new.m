classdef Spike_adaptive_new
    ...
    properties
        ...
        contrast (1,1) {mustBeNumeric} = 0.0008;
        found_fit = false;
    ...
    properties (Hidden)
        ...
        num_initial_projs % = obg.smp_freqs * CS_PER_MEASURE; % (1,1) {mustBeNumeric}
        % number of intial projections before we start reconstructions
        num_max_projs % = obj.smp_freqs * CS_PER_MEASURE; % (1,1) {mustBeNumeric}
        % lorentzian kernel
        basis_kernel 
    ...
    methods
        ...
        function obj = Spike_adaptive_new(freqs, num_sample_freqs, contrast, window_start, window_end, num_of_measures, varargin)
            ...
            obj.num_measures = num_of_measures;
            obj.num_initial_projs = num_of_measures;
            obj.num_max_projs = num_of_measures;
            
%             obj.num_measures = ceil(obj.num_pts * obj.CS_PER_MEASURE);
%             obj.num_initial_projs = ceil(obj.num_pts * obj.CS_PER_MEASURE);
%             obj.num_max_projs = ceil(obj.num_pts * obj.CS_PER_MEASURE);
...
%               obj.num_max_projs = ceil(obj.CS_PER_MEASURE * length(obj.smp_freqs));
              obj.num_max_projs = num_of_measures;
...
%             end
            ...
            obj.all_means = zeros(ceil(obj.num_max_projs),1);
        ...
        function [obj, freqs_to_measure] = frequency(obj)
            ...
            elseif obj.Continue
                ...
                elseif obj.poll_window
                    ...
                    % DVIR CHANGE TRY TO MAKE SIMPLE NON-ADDAPTIVE MATRIX (DELETE THESE ROWS).
                    obj.smp_mtx = [obj.smp_mtx; new_row];  % add to sampling matrix
                    obj.measurement_number=obj.measurement_number+1;
                    freqs_to_measure(1:obj.num_sample_freqs) = ...
                        obj.smp_freqs(obj.smp_mtx(obj.measurement_number,:)>0);
                ...
                % WOWWWWWWW
%                 obj.smp_mtx = generateSamplingMtx(obj.num_pts, 100, 'On/Off Spikes', 'params');
            ...
            if (obj.flag_start_reconstructions) && (~obj.fit_converged || mod(obj.num_second_pass,3)==0)
                ...
                try
                    ...
                    if obj.get_err && obj.sim.sigma==0
                        sigma = 0.0000001;%000010.0000001
...
%                     if obj.display
                    ...
                    if obj.measurement_number>=obj.num_max_projs
                        ...
                        obj.pk_locs = sort(obj.curr_locs); % CHANGE!!!
...
%             prob_to_poll_peak = 1-(1-prob_to_poll_peak)^obj.; %at least one frequency
%             
...
% y_data = data;
% x_data = linspace(1 , length(y_data) , length(y_data))';
smooth_data = smooth(data,smoothing,'loess'); %kills the hyperfine, we dont need it.
% smooth_data = smooth(x_data, y_data, smoothing,'loess'); %kills the hyperfine, we dont need it.
[~,locs,width,prominence] = findpeaks(smooth_data ,'MinPeakHeight', contrast);%in the last parameter insert the contrast of the smaiiest peak
num_pks=length(locs);
